...
int InitToStartRun(int openDataFile) {
  ...
  if (openDataFile) {
    ...
    if (strcmp(CSMDataFilename, "/dev/null") != 0) {
      CSMDataFile_1 = fopen(CSMDataFilename, "wb");
      if (CSMDataFile_1 != NULL) printf("Data file <%s> is opened.\n", CSMDataFilename);
      else printf("Unable to open data file <%s>.\n", CSMDataFilename);
    }
    rawDataFile_1 = NULL;
    if (strlen(rawDataFilename) > 4) {
      rawDataFile_1 = fopen(rawDataFilename, "wb");
      if (rawDataFile_1 != NULL) printf("Raw Data file <%s> is opened.\n", rawDataFilename);
      else printf("Unable to open raw data file <%s>.\n", rawDataFilename);
    }
    logFile_1 = NULL;
    logFile_1 = fopen("socket.log","w");
    if (logFile_1 == NULL) printf("Unable to open log file.\n");

    CSMDataFile_2 = NULL;
    if (strcmp(CSMDataFilename, "/dev/null") != 0) {
      strcpy(CSMDataFilename_2,CSMDataFilename);
      strcat(CSMDataFilename_2,"_2");
      CSMDataFile_2 = fopen(CSMDataFilename_2, "wb");
      if (CSMDataFile_2 != NULL) printf("Data file <%s> is opened.\n", CSMDataFilename_2);
      else printf("Unable to open data file <%s>.\n", CSMDataFilename_2);
    }
    rawDataFile_2 = NULL;
    if (strlen(rawDataFilename) > 4) {
      strcpy(rawDataFilename_2,rawDataFilename);
      strcat(rawDataFilename_2,"_2");
      rawDataFile_2 = fopen(rawDataFilename_2, "wb");
      if (rawDataFile_2 != NULL) printf("Raw Data file <%s> is opened.\n", rawDataFilename_2);
      else printf("Unable to open raw data file <%s>.\n", rawDataFilename_2);
    }
    logFile_1 = NULL;
    logFile_1 = fopen("socket.log","w");
  ...
  p_CollectCSMData_1 = new CollectCSMData::CollectCSMData(1, CSMDataFile_1, rawDataFile_1, logFile_1);
  p_CollectCSMData_2 = new CollectCSMData::CollectCSMData(2, CSMDataFile_2, rawDataFile_2, logFile_2);
...
void PrintAnalysisCounter(FILE *file, int cntIndex, char name[], int maxLength) {
  ...
  if (length > 0) {
    ...
    if ((high >= 0) && (p_CollectCSMData_1->analysisCounter[high] > 0)) {
      sum = maxUintValue * ((double) p_CollectCSMData_1->analysisCounter[high]) + (double) p_CollectCSMData_1->analysisCounter[low];
      ...
      if (file == NULL) printf(str, name, ' ', p_CollectCSMData_1->analysisCounter[cntIndex]);
      else fprintf(file, str, name, ' ', p_CollectCSMData_1->analysisCounter[cntIndex]);
    }
  }
}
...
void SaveErrorSummaryFile(void) {
  ...
  if (p_CollectCSMData_1->nbMezzCard <= 1) {
    if (validBCIDPresetValue == 0 || p_CollectCSMData_1->nbMezzCard <= 0) {
  ...
  if (sumFile = fopen(sumFilename, "w")) {
    ...
    if ((p_CollectCSMData_1->analysisCounter[WRONGEVENT] > 0 || p_CollectCSMData_1->analysisCounter[WARNINGEVENT] > 0)) {
      for (i = 0; i < NUMBERANALYSISCOUNTER; i++) 
        PrintAnalysisCounter(sumFile, i, name[i], maxLength);
    }
    else {
      for (i = 0; i < NUMBERANALYSISCOUNTER; i++)
        if (p_CollectCSMData_1->analysisCounter[i] > 0) PrintAnalysisCounter(sumFile, i, name[i], maxLength);
...
void SaveTDCTimeSpectrum(void) {
  ...
  if (p_CollectCSMData_1->analysisCounter[ANALYSEDEVENTHIGH] > 0) return;
  if (p_CollectCSMData_1->analysisCounter[ANALYSEDEVENT] == 0) return;
    ...
    for (mezz = 0; mezz < MAXNUMBERMEZZANINE; mezz++) {
      for (ch = 0; ch < 24; ch++) {
        ...
        if (nlist > 0) {
          ...
          for (i = 0; i < nlist; i++) {
            fprintf(spectrumFile, "%11d", p_CollectCSMData_1->timeSpectrum[i][ch][mezz]);
        ...
        nlist = 0;
        for (i = 512; i > 0; i--) {
          if (p_CollectCSMData_1->timeWidth[i-1][ch][mezz] > 0) {
             nlist = i;
             break;
          }
        }
        if (nlist > 0) {
          ...
          fprintf(spectrumFile, "Number of entry : %d\n", nlist);
          for (i = 0; i < nlist; i++) {
            fprintf(spectrumFile, "%11d", p_CollectCSMData_1->timeWidth[i][ch][mezz]);
            if (((i%8) == 7) || (i == (nlist-1))) fprintf(spectrumFile, "\n");
          }         
        }
      }
...
void CalculateAverageTDCTimeAndSigma(void) {
  ...
  for (mezz = 0; mezz < MAXNUMBERMEZZANINE; mezz++) {
    for (ch = 0; ch < 24; ch++) {
      ...
      nEntry = 0;
      for (i = 0; i < 4000; i++) {
        entry = p_CollectCSMData_1->timeSpectrum[i][ch][mezz];
        if (entry > nEntry) {
          imax1 = i;
          nEntry = entry;
        }
      ...
      i0 = imax1;
      while (i0 > 0) {
        entry = p_CollectCSMData_1->timeSpectrum[i0][ch][mezz];
        if (p_CollectCSMData_1->timeSpectrum[i0-1][ch][mezz] < entry) i0--;
        else {
          if (i0 > 3) {
            if ((p_CollectCSMData_1->timeSpectrum[i0-2][ch][mezz] >= entry) && (p_CollectCSMData_1->timeSpectrum[i0-3][ch][mezz] >= entry)) break;
            else i0--;
          }
          else i0 = 0;
        }
      ...
      i1 = imax1;
      while (i1 < 3999) {
        entry = p_CollectCSMData_1->timeSpectrum[i1][ch][mezz];
        if (p_CollectCSMData_1->timeSpectrum[i1+1][ch][mezz] < entry) i1++;
        else {
          if (i1 < 3996) {
            if ((p_CollectCSMData_1->timeSpectrum[i1+2][ch][mezz] >= entry) && (p_CollectCSMData_1->timeSpectrum[i1+3][ch][mezz] >= entry)) break;
            else i1++;
          }
          else i1 = 3999;
        }
      ...
      if (nEntry > 0) {
        ...
        for (i = 0; i < 4000; i++) {
          ...
          sum += ((double) p_CollectCSMData_1->timeSpectrum[i][ch][mezz]) * average * average;
        }
...
void CalculateAverageWidthAndSigma(void) {
  ...
  for (mezz = 0; mezz < MAXNUMBERMEZZANINE; mezz++) {
    for (ch = 0; ch < 24; ch++) {
      ...
      sum = 0.0;
      for (i = 0; i < 511; i++) {
        entry = p_CollectCSMData_1->timeWidth[i][ch][mezz];
        nEntry += entry;
        sum += ((double) entry) * ((double) i);
      }
      subEntry = (double) nEntry;
      totalEntry = (double) nEntry + (double) p_CollectCSMData_1->timeWidth[511][ch][mezz];
      if (nEntry > 0) {
        ...
        for (i = 0; i < 511; i++) {
          ...
          sum += ((double) p_CollectCSMData_1->timeWidth[i][ch][mezz]) * average * average;
        }
        average = sum / ((double) nEntry);
...
void ChildSigTERMHandler(int sig) {
  ...
  if (CSMDataFile_1 != NULL) {
    fclose(CSMDataFile_1);
    printf("\nData file <%s> is closed.\n", CSMDataFilename);
    CSMDataFile_1 = NULL;
  }
  if (rawDataFile_1 != NULL) {
    fclose(rawDataFile_1);
    printf("\nRaw Data file <%s> is closed.\n", rawDataFilename);
    rawDataFile_1 = NULL;
  }

  if (CSMDataFile_2 != NULL) {
    fclose(CSMDataFile_2);
    printf("\nData file <%s> is closed.\n", CSMDataFilename_2);
    CSMDataFile_2 = NULL;
  }
  if (rawDataFile_2 != NULL) {
    fclose(rawDataFile_2);
    printf("\nRaw Data file <%s> is closed.\n", rawDataFilename_2);
    rawDataFile_2 = NULL;
  }
