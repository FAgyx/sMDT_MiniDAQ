...
int pipe1[2], pipe2[2], clientID, CSMNumber, sock, normalQuit, nbRequestedBuf, bufnr;
unsigned int buffer[4*BUFFER_MAX], prevBuffer[4*BUFFER_MAX];         // incoming data with 8bit valid data
unsigned int totalDAQTime, toBePaused;
	  ...
	  while (connected) {
		...
		if (TCPData[0] != 0) {
		  ...
		  if (status == 0) {
			if (command == CMDDETAILSTATUS) {
			...
			else if (command == CMDPAUSERUN) {
			  DAQState = State_Paused;
			}
			  ...
			  i = TCPData[0];
			  if ((command == CMDREQDAQINFOR) && (DAQState != State_Idle)) {
				kill(cpid, SIGUSR1);
			  } else {
...
void CollectCSMData(void)
{
  ...
  while (DAQState != State_Idle) {
	...
	while (DAQState != State_Idle) {
	  ...
	  if (nfifos >= numberFilledFIFOs)
		numberFilledFIFOs = nfifos;
	  else if (numberFilledFIFOs >= 15)
		numberFilledFIFOs = 16+nfifos;
	  ...
	  for (i = 0; i < datasize; i++) {
		...
		if (synchWord) {
		  ...
		  if (checkExpectedData) {
			if (wordID == TDC_HEADER) {             // got a TDC header
			  nReceivedData[mezz] = 0;
			  nUnexpected[mezz] = 0;
			}
			else if (wordID == TDC_TRAILER) {       // got a TDC trailer
			...
			else if (wordID == TDC_EDGE) {       // got a TDC egde word
			  nReceivedData[mezz]++;
			...
			else if ((wordID == TDC_PAIR) || (wordID == TDC_MASK) || (wordID == TDC_ERROR) || (wordID == TDC_DEBUG)) {
			  nReceivedData[mezz]++;
			  receivedData = *(dataptr + i) & 0xFFFFFFF;
			  expected = 8*nReceivedData[mezz] | (1<<STYPEBIT0LOCATION);
			  if (((receivedData-expected)&(~ODDPARITYBITS)) == 0) nUnexpected[mezz]++;
		  ...
		  if (mezzCardEnable[mezz]) {
			if (wordID == TDC_HEADER) {             // got a TDC header
			...
			else if (wordID == TDC_MASK) {         // got a TDC mask word
			  nTDCMask[mezz]++;
			...
			else if (wordID == TDC_ERROR) {        // got a TDC error word
			  ...
			  if ((data&0x3FFF) == 0) {
				...
				if (AMTType == TYPEAMT1) {
				  ...
				  else if ((data&0x3E00) != 0) singleCounter[TDCSOFTERROR]++;
				}
				for (j = 0; j < 14; j++) {
				  if ((data&(1 << j)) != 0) {
			...
			else if (wordID == TDC_DEBUG) {      // got a TDC debug word
			  nTDCDebug[mezz]++;
			  ...
			  edge = (data>>STYPEBIT0LOCATION) & STYPEBITS;
			  if (collectHits) nEdge[edge][chan][mezz]++;
			  if (edge == 1) {
				if (tdcTime < 4000) {
				  timeSpectrum[tdcTime][chan][mezz]++;
				  //printf("tdcTime: %d\n", tdcTime);
				  //printf("previous tdcTime: %d\n", previousTDCTime[chan][mezz]);
				}
			...
			else if (wordID == TDC_PAIR) {        // got a TDC pair word
			  nTDCPair[mezz]++;
			  nTDCHits[mezz][evtWRBuf[mezz]]++;
			  chan = (data>>CHANNELNUMBERBIT0LOCATION) & CHANNELNUMBERBITS;
			  tdcTime = (data>>PTDCTIMEBIT0LOCATION) & PTDCTIMEBITS;
			  if (tdcTime <= previousTDCTime[chan][mezz]) {
				evtWarning[evtWRBuf[mezz]]++;
				singleCounter[TDCTIMEWRONGORDER]++;
				singleCounter[TDCTIMEWRONGORDERINTDC+mezz]++;
			  ...
			  width = (data>>PWIDTHBIT0LOCATION) & PWIDTHBITS;
			  timeWidth[width][chan][mezz]++;
			  if (collectHits) {
				nPair[chan][mezz]++;
				if (width > minWidth) nGoodHit[chan][mezz]++;
			...
			if (bufEmpty[evtWRBuf[mezz]] && (data == previousData[mezz])) {
			  ...
			  if (suppressCSMHeader && suppressCSMTrailer) {
				...
				eventBuf[0][evtWRBuf[mezz]] = 2;
				if ((wordID == TDC_HEADER) || (wordID == TDC_TRAILER)) eventBuf[1][evtWRBuf[mezz]] = (evid) << EVIDBIT0LOCATION;
				else eventBuf[1][evtWRBuf[mezz]] = (evtNumber%4096) << EVIDBIT0LOCATION;
				evtWRIndex[evtWRBuf[mezz]] = 2;
			  }
			  if ((wordID == TDC_HEADER) || (wordID == TDC_TRAILER)) bufEVID[evtWRBuf[mezz]] = evid;
			...
			if (countData)
			  evtSize[mezz][evtWRBuf[mezz]]++;
			if (wordID == TDC_TRAILER) {           // got a TDC trailer
			  ...
			  if (gotTrailer[evtWRBuf[mezz]] == mezzEnables) {  // Event ready
				  ...
				  while (evtWRBuf[mezz] != evtRDBuf) {
					...
					if (CSMDataFile != NULL) {
					  if ((nDataWords+12+nwords) > MAXEVTRECORD) {
						...
						else fwrite(dataBuf, sizeof(unsigned int), nDataWords+11, CSMDataFile);
						nPackedEvt = 0;
						nDataWords = 0;
					  }
					...
					evtRDBuf++;
					evtRDBuf %= 64;
					if (evtNumber > 20000 && evtNumber < 20500) {
					  int maxbin_1 = 0;
					  int maxbin_7 = 0;
					  for(int tj=1; tj<4000; tj++){
						if(timeSpectrum[tj][21][1]>timeSpectrum[maxbin_1][21][1]) maxbin_1 = tj;
						if(timeSpectrum[tj][21][7]>timeSpectrum[maxbin_7][21][7]) maxbin_7 = tj;
					  }
					  bool jump1 = (maxbin_1-213)>4 || (213-maxbin_1)>4;
					  bool jump7 = (maxbin_7-228)>4 || (228-maxbin_7)>4;
					  if( jump1 || jump7 ) {
						printf("max bin for mezzanine 1: %d\n", maxbin_1);
						printf("max bin for mezzanine 7: %d\n", maxbin_7);
						toBePaused = true;
					  }
				...
				if (evtNumber > 25000 && evtNumber < 26000) {
				  int maxbin_1 = 0;
				  int maxbin_7 = 0;
				  for(int tj=1; tj<4000; tj++){
					if(timeSpectrum[tj][21][1]>timeSpectrum[maxbin_1][21][1]) maxbin_1 = tj;
				  ...
				  if ( evtNumber == 25001 && (maxbin_1 != 218 || maxbin_7 != 233) ) {
					printf("\nMax bin for mezzanine 1: %d\n", maxbin_1);
					printf("Max bin for mezzanine 7: %d\n", maxbin_7);
				  }
				  bool jump1 = (maxbin_1-218)>1 || (218-maxbin_1)>1;
				  bool jump7 = (maxbin_7-233)>1 || (233-maxbin_7)>1;
				  if( jump1 || jump7 || singleCounter[WRONGEVENT]>0 ) {
					toBePaused = true;
				  }
				} else {
				  toBePaused = false;
			...
			{             // got a TDC header
			  ...
			  if (wordID == TDC_EDGE) nTDCEdge[mezz]++;
			  else if (wordID == TDC_PAIR) nTDCPair[mezz]++;
			  else if (wordID == TDC_MASK) nTDCMask[mezz]++;
			  else if (wordID == TDC_ERROR) nTDCError[mezz]++;
			  else if (wordID == TDC_DEBUG) nTDCDebug[mezz]++;
