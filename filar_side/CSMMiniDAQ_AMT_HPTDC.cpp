  ...
  while (DAQState != State_Idle) {
    ...
    if (CSMVersion%2 == 1) {
      ...
      for (i = 0; i < datasize; i++) {
        data = *(dataptr + i);
	if ((data&CSMPARITYERRORBIT) != 0) {
        //if ((data&CSMPARITYERRORBIT) != CSMPARITYERRORBIT && data!=CSMIDLEWORD&&(data&SWORDHEADER) !=SWORDHEADER) {
        ...
        if (synchWord) {
          ...
          if (checkExpectedData) {
            if (wordID == HPTDC_HEADER||wordID == TDC_HEADER) {             // got a TDC header
              nReceivedData[mezz] = 0; 
              nUnexpected[mezz] = 0; 
            }
            else if (wordID == HPTDC_TRAILER||wordID == TDC_TRAILER) {       // got a TDC trailer
          ...
          if (mezzCardEnable[mezz]) {
            ...
            if (bufEmpty[evtWRBuf[mezz]] && (data == previousData[mezz])) {
              countData = FALSE;
              copyData = FALSE;
              if (wordID == HPTDC_HEADER||wordID == TDC_HEADER) {
                singleCounter[EXTRATDCHEADER]++;
                singleCounter[TDCHEADEREXTRATDCID+mezz]++;
              }
              else if (wordID == HPTDC_TRAILER||wordID == TDC_TRAILER) {
                singleCounter[EXTRATDCTRAILER]++;
                singleCounter[TDCTRAILEREXTRATDCID+mezz]++;
              }
            }
              ...
              if (suppressCSMHeader && suppressCSMTrailer) {
                ...
                eventBuf[0][evtWRBuf[mezz]] = 2;
                if ((wordID == HPTDC_HEADER||wordID == TDC_HEADER) || (wordID == HPTDC_TRAILER||wordID == TDC_TRAILER)) eventBuf[1][evtWRBuf[mezz]] = (evid) << EVIDBIT0LOCATION;
                else eventBuf[1][evtWRBuf[mezz]] = (evtNumber%4096) << EVIDBIT0LOCATION;
                evtWRIndex[evtWRBuf[mezz]] = 2;
              }
              if ((wordID == HPTDC_HEADER||wordID == TDC_HEADER) || (wordID == HPTDC_TRAILER||wordID == TDC_TRAILER)) bufEVID[evtWRBuf[mezz]] = evid;
            ...
            if (wordID == HPTDC_TRAILER||wordID == TDC_TRAILER) {           // got a TDC trailer
              ...
              if (checkCSMOverflows) { 
                ...
                //diff = wc - evtSize[mezz][evtWRBuf[mezz]]+2;
...
////////////////////debug gyx
		//    printf("trail = %d, evtsize = %d\n, mismatch counter = %d",wc,evtSize[mezz][evtWRBuf[mezz]],singleCounter[TDCWCMISMATCHLIST+mezz]);
                  }
                  else {
...
void RawDataInterpretation(unsigned int data, FILE *file) {
  ...
  if (wordID == ID_SYNCHWORD) {
    ...
    else if (wordID == HPTDC_HEADER||wordID == TDC_HEADER) {
