...
unsigned int totalDAQTime, timeJumped;
      ...
      for (i = 0; i < datasize; i++) {
        ...
        if (synchWord) {
          ...
          if (checkExpectedData) {
            ...
            else if ((wordID == TDC_PAIR) || (wordID == TDC_MASK) ||
                     (wordID == TDC_ERROR) || (wordID == TDC_DEBUG)) {
              nReceivedData[mezz]++;
              receivedData = *(dataptr + i) & 0xFFFFFFF;
              expected = 8*nReceivedData[mezz] | (1<<STYPEBIT0LOCATION);
              if (((receivedData-expected)&(~ODDPARITYBITS)) == 0) nUnexpected[mezz]++;
		  ...
		  evtCSMFIFOOV[j][evtWRBuf[mezz]] = 0;
		    ...
		    if ((*(dataptr+j)&SWORDHEADERMASK) == SWORDHEADER) break;
		      ...
		      //                      bool n1 = (maxbin_1==216);
		      //                      bool n7 = (maxbin_7==229);
		      //                      if( n1 && n7 ){
/*
			bool jump1p = (maxbin_1>234 && maxbin_1<240);

			bool jump7p = (maxbin_7>246 && maxbin_7<252);

			bool jump1m = (maxbin_1>202 && maxbin_1<208);

			bool jump7m = (maxbin_7>214 && maxbin_7<220);
//			printf(" maxbin_1 is %d and maxbin_7 is %d\n",maxbin_1,maxbin_7);
		   if( (jump1p && jump7p) || (jump1m && jump7m) ){
*/
// Normal values on 11/24/14 are 221 and 237
// New normal values on 12/12 are 215 and 226
		   bool jump1p = (maxbin_1 > 224);
		   bool jump1m = (maxbin_1 < 206);
		   bool jump7p = (maxbin_7 > 235);
		   bool jump7m = (maxbin_7 < 217);
		   if ( jump1p || jump7p || jump1m || jump7m ) {
                        timeJumped = TRUE;
                      }
                    }
