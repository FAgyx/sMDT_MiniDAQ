...
int nbMezzCard, validBCIDPresetValue, mezzCardEnable[MAXNUMBERMEZZANINE], reqEVIDMatch, AMTType;
...
unsigned int dataBuf[MAXEVTRECORD], eventBuf[MAXEVTRECORD][64], evtRDBuf, bufEmpty[64];
unsigned int evtWRBuf[MAXNUMBERMEZZANINE], evtWRIndex[64], evtSize[MAXNUMBERMEZZANINE][64];
unsigned int analysisCounter[NUMBERANALYSISCOUNTER], singleCounter[NUMBERANALYSISCOUNTER];
unsigned int numberFilledFIFOs, requestForPause, requestForStop, synchWordControl;
...
FILE *CSMDataFile, *rawDataFile, *logFile;
...
int saveRunCondition, CSMPairDebug, checkSystemNoise, mappingMDTChamber, startMezzCard;
int suppressCSMHeader, suppressCSMTrailer, suppressTDCHeader, suppressTDCTrailer;
int AMTDataMode0, AMTDataMode1, MDTChamberType, MDTChamberHVStatus, saveFlaggedEvent, saveRawData;
