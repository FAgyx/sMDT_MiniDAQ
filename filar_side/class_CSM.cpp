  ...
  if(filar_chnl_no == 1){//configure CSM1 from TCP command
    ...
    for(i = 0; i < MAXNUMBERMEZZANINE; i++) {
      ...
      if(i < 10) {
        ...
        lEdgeOn[i] = (buffer[TCPEDGEPAIRENABLES1] >> (3*(i%10) + LEADINGEDGEENABLE)) & 1;
        tEdgeOn[i] = (buffer[TCPEDGEPAIRENABLES1] >> (3*(i%10) + TRAILINGEDGEENABLE)) & 1;
        pairOn[i] = (buffer[TCPEDGEPAIRENABLES1] >> (3*(i%10) + PAIRENABLE)) & 1;
      }
    }
  }
