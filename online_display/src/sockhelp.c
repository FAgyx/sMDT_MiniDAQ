...
int make_connection( char *service, int type, char *netaddress)
{
  ...
  int port = -1;
  struct in_addr *addr;
  int sock, connected, b;
  struct sockaddr_in address;
  char tcp_str[]="tcp";
  char udp_str[]="udp";

  if (type == SOCK_STREAM) 
    port = atoport(service, tcp_str);
  if (type == SOCK_DGRAM)
    port = atoport(service, udp_str);
  if (port == -1) {
...
/* This is just like the read() system call, accept that it will make
   ...
   Note: it has been modified to behave like the sock_gets() routine.
         it will return a -1 if the socket is closed during the read 
         operation - joe */
