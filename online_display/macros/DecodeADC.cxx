...
int DecodeADC(TString filename = "run00187939_20190514.dat") {
  //gROOT->SetBatch(kTRUE); // set to batch mode to inprove the speed

  
  // open input file
  TString input_filename = "raw/";
  ...
  int nloop = 0;
  while (data_in_flow.read((char *) &word, sizeof(word))  && nloop<1000000) {
    nloop++;
    ...
    if (header_type == Signal::GROUP_HEADER || header_type == Signal::TDC_HEADER || header_type == Signal::TDC_TRAILER) {
      ...
      if (currEventID.ID() != prevEventID.ID()) {
      	...
      	if (total_events %10000 == 0) cout << "Processing Event " << total_events << endl;
  ...
  #ifdef SAVE_TRACKS_OUT_OF_ROOT
  TCanvas *p_output_canvas = new TCanvas("c1", "ADC Plots");
  p_output_canvas->Divide(2,2);
  for (Int_t tdc_id = 0; tdc_id != Geometry::MAX_TDC; tdc_id++) {
